...
(function(DayPilot) {
    ...
    sheet.add(".modal_default_form_item input", "width: 100%; box-sizing: border-box;");
    ...
    // sheet.add(".modal_default_form_item_date:after", "content: '\\25bc'; position: absolute; right: 5px; bottom: 1px;");
...
*/
     ...
     */
    var getFieldType = function(item) {
        var known = ["text", "date", "select"];
    ...
    var createElement = function(item, modal) {
        var elements = {
            "text": function(modal) {
                var input = document.createElement("input");
                // input.id = item.id;
            ...
            "date": function(modal) {
                ...
                var input = document.createElement("input");
                input.className = modal.theme + "_input_date";
                // input.id = item.id;
                input.name = item.id;
                // input.value = item.def || "";
                input.onkeydown = function(e) {
                    ...
                    switch (e.keyCode) {
                        ...
                        case 27: // escape
                            ...
                            break;
                        case 9:  // tab
                            input.picker && input.picker.close();
                            letcontinue = true;
                            break;
                        default:
                            letcontinue = true;
                ...
                var Picker = DayPilot.DatePicker || DayPilot.ModalDatePicker;
                ...
                if (item.dateFormat) {
                    picker.pattern = item.dateFormat;
                }
                var locale = item.locale || modal.locale;
                if (locale) {
        ...
        var type = getFieldType(item);
        return elements[type](modal);
    };
    ...
    var createFormField = function(modal, item) {
        ...
        var isInteractive = typeof item.id === "string";
        var type = getFieldType(item);

        if (isInteractive) {
            input = createElement(item, modal);
        }
        ...
        if (isInteractive) {
            div.className += " " + modal.theme + "_form_item_" + type;
        }
        ...
        field.set = function(value) {
            if (field.element) {
                if (field.type === "date") {
                    var picker = field.element.picker;
                    picker.date = new DayPilot.Date(value);
                    var formatted = new DayPilot.Date(value).toString(item.dateFormat || picker.pattern, picker.locale);
                    field.element.value = formatted;
                }
                else {
