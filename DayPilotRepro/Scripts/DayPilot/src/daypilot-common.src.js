...
*/
    ...
    DayPilot.contains = function(array, object) {
        ...
        if (arguments.length !== 2) {
            throw new DayPilot.Error("DayPilot.contains() requires two arguments.");
        }
...
*/
    ...
    DayPilot.ua = function(array) {
        ...
        if (!DayPilot.isArray(array)) {
            throw new DayPilot.Error("DayPilot.ua() - array required");
        }
    ...
    DayPilot.Util.fontHeight = function(font) {
        ...
        if (typeof DayPilot.Util.fontHeight.cache[fs] !== "number") {
            ...
            document.body.removeChild(div);
            // console.log("font height:", fs, DayPilot.Util.fontHeight.cache[fs]);
        }
...
/*    DayPilot.Http.post = function(object) {
    ...
    DayPilot.Canvas = function(width, height, format, scale, jpegQuality) {
        ...
        /*        this.upperLeftCorner = function(rect, color) {
            ...
            ctx.beginPath();
            ctx.moveTo(rect.x + 0.5, rect.y - 0.5);
            ctx.lineTo(rect.x + rect.w - 0.5, rect.y - 0.5);
            ctx.lineTo(rect.x + 0.5, rect.y + rect.h - 0.5);
            ctx.closePath();
            ctx.fill();
        ...
        this.upperRightCorner = function(rect, color) {
            ...
            ctx.moveTo(rect.x + 0.5, rect.y - 0.5);
            ctx.lineTo(rect.x + rect.w - 0.5, rect.y - 0.5);
            ctx.lineTo(rect.x + rect.w - 0.5, rect.y + rect.h - 0.5);
            ctx.closePath();
            ctx.fill();
    ...
    DayPilot.Svg = function(width, height) {
        ...
        this.groupEnd = function() {
            if (!this.group) {
                throw new DayPilot.Error("SVG export = group not open");
            }
        ...
        this.image = function(rect, img) {
            ...
            function doit(args) {
                ...
                // Causes error: "Namespace prefix xlink for href on image is not defined", Chrome doesn't include the xlink namespace definition in SVG tag
                // image.setAttributeNS(xlink, "xlink:href", args.dataUri);
...
*/
    ...
    (function() {
        ...
        sheet.add(".scheduler_default_rowheader_inner", "position: absolute;left: 0px;right: 0px;top: 0px;bottom: 0px;border-right: 1px solid #e0e0e0;padding: 2px;display: flex; align-items: center;");
        ...
        sheet.add(".scheduler_default_event_float_inner:after", 'content:"";border-color: transparent #666 transparent transparent;border-style:solid;border-width:5px;width:0;height:0;position:absolute;top:4px;left:-4px;');
        ...
        sheet.add(".scheduler_default_main .scheduler_default_event_group", "box-sizing: border-box; font-size:12px; color:#666; padding:2px; overflow:hidden; border:1px solid #ccc; background-color: #fff; display: flex; align-items: center; white-space: nowrap;");
        ...
        sheet.add(".scheduler_default_shadow_invalid .scheduler_default_shadow_inner", "background-color: red;");
        ...
        // 2018-07-24 experimental
        sheet.add(".scheduler_default_main:focus", "outline: none;");
        sheet.add(".scheduler_default_cell_focus", "outline: red 2px solid; z-index: 100; opacity: 0.5;");
        ...
        // 2019-12-03 updated
        // sheet.add(".scheduler_default_rowheader.scheduler_default_rowheader_selected", "background-color: #aaa; background-image: linear-gradient(45deg, rgba(255, 255, 255, .2) 25%, transparent 25%, transparent 50%, rgba(255, 255, 255, .2) 50%, rgba(255, 255, 255, .2) 75%, transparent 75%, transparent); background-size: 35px 35px;");
        // sheet.add(".scheduler_default_cell.scheduler_default_cell_business.scheduler_default_cell_selected,.scheduler_default_cell.scheduler_default_cell_selected", "background-color: #ccc; background-image: linear-gradient(45deg, rgba(255, 255, 255, .2) 25%, transparent 25%, transparent 50%, rgba(255, 255, 255, .2) 50%, rgba(255, 255, 255, .2) 75%, transparent 75%, transparent); background-size: 20px 20px;");
        ...
        sheet.add(".gantt_default_rowheader_inner", "position: absolute;left: 0px;right: 0px;top: 0px;bottom: 0px;border-right: 1px solid #e0e0e0;padding: 2px;display: flex; align-items: center;");
        ...
        sheet.add(".gantt_default_event_left", "white-space: nowrap; padding-top: 5px; color: #666; cursor: default;  display: flex; align-items: center;");
        sheet.add(".gantt_default_event_right", "white-space: nowrap; padding-top: 5px; color: #666; cursor: default;  display: flex; align-items: center;");
        ...
        // 2019-09-16
        // timeheadergroup_inner, timeheadercol_inner (flex alignment)
        // sheet.add(".gantt_default_divider, .gantt_default_splitter", "background-color: #c0c0c0;");
        // sheet.add(".gantt_default_divider, .gantt_default_splitter", "background-color: #c0c0c0;");
        // sheet.add(".gantt_default_timeheadergroup_inner", "position: absolute;left: 0px;right: 0px;top: 0px;bottom: 0px;border-right: 1px solid #c0c0c0;border-bottom: 1px solid #c0c0c0;display: flex; align-items: center; justify-content: center;");
        // sheet.add(".gantt_default_timeheadercol_inner", "position: absolute;left: 0px;right: 0px;top: 0px;bottom: 0px;border-right: 1px solid #c0c0c0;display: flex; align-items: center; justify-content: center;");
    ...
    var Splitter = function(id) {
        ...
        resolved.splitterWidth = function() {
            return This.enabled ? This.splitterWidth : 1;
        };
        ...
        this.init = function() {
            ...
            for (var i = 0; i < this.widths.length; i++) {
                ...
                s.style.width = (this.widths[i] - resolved.splitterWidth()) + "px";
                ...
                handle.style.width = resolved.splitterWidth() + "px";
        ...
        this._updateWidth = function(i) {
            var block = this.blocks[i];
            var width = this.widths[i];
            block.section.style.width = (width - resolved.splitterWidth()) + "px";
        };
